...
- [x] **Confirm the offered slots are correct on the live site** — DONE 2026-08-22. Live site offers 9:00 AM–4:30 PM Pacific, no 2 AM slots. Original: after setting timezone and hours there. Local and production have separate databases.
- [x] One local date (2026-08-31) still showed as bookable — superseded; now reproduced on production and tracked properly under Round 3 below. Original: in the month grid while returning zero slots. Most likely a stale 5-minute KV cache entry populated before the fix above, but it was not conclusively re-tested after the TTL expired. Worth a look if the symptom reappears on production.
...
- [x] **Booking page dates briefly all-disabled after an availability change.** DIAGNOSED AND MITIGATED (86745c3). Cause: Cloudflare KV is eventually consistent, so for ~60s after a save a request can read the OLD cache version and serve pre-change data. Cached entries dropped from 300s to 60s (KV's minimum) to bound the window. Proper fix — move the version to D1, which is strongly consistent — is documented in `src/lib/server/availability-cache.ts`. Original: Reported during the same test: right after saving new hours, the calendar showed every date disabled; one reload made them selectable. Likely the 300s KV cache — the version bump should prevent this, so either the client held stale data or the invalidation did not fire on that path. Not reproduced since. Watch for it; if it recurs, check whether the dashboard availability save actually reaches `invalidateAvailabilityCache`.
...
- [x] **Reschedule confirmation screen offers no way to reschedule again.** FIXED (4a33fe1) — Reschedule link added beside Cancel, same styling, less-destructive option first. Original: After rescheduling, the screen shows "Join Google Meet" and "Cancel" but no "Reschedule" link. A second reschedule is possible — it works via the URL, which is what the emailed link provides — but an attendee looking at that screen would conclude they can only cancel. Add a Reschedule link alongside Cancel. File: `src/routes/reschedule/[id]/+page.svelte`.

- [x] **A day can appear bookable in the month grid but offer zero times.** FIXED (72180a8). Root cause was an off-by-one on the month's upper bound: `lastDay = new Date(year, monthNum, 0)` is local midnight at the *start* of the last day, and it was used as the upper bound for the Google/Outlook freeBusy queries AND the bookings SQL. An event on the 31st starting at 09:00 fell outside `[firstDay, lastDay]`, so the month grid could not see it and marked a fully-busy day bookable. Proven with debug logging against a real calendar: the day endpoint found busy `2026-08-31T07:00:00Z`–`23:59:59Z` while the month query window ended at exactly `07:00:00.000Z`. Fixed by using `rangeEnd = new Date(year, monthNum, 1)` (start of the following month) and switching the SQL to a half-open `start_time < rangeEnd`. The duplicated slot-generation loop was also extracted into `src/lib/server/availability-slots.ts` and is now shared by both endpoints, removing the drift that already caused commit e0c4437. Verified: 38 dates across Aug/Sep/Oct, zero mismatches. Note `2026-09-14` was a red herring — both endpoints correctly agreed it was unavailable. Original: Reproduced on production: `2026-08-31` is listed in `/api/availability/month` yet `/api/availability?date=2026-08-31` returns `{"slots":[]}`. Same for `2026-09-14` locally. Both endpoints do check calendar busy times and their slot logic is equivalent, so the leading hypothesis is that the **month-wide Google freeBusy query returns a truncated set of busy periods**, causing the grid to under-detect conflicts, while the single-day query sees them all. Not confirmed — it would need a look at the host's actual calendar for those dates. Impact is mild: a visitor clicks a highlighted day and sees "No available times for this date", then picks another. Worth fixing, not worth blocking on.
